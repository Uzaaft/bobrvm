...
const pci_device_count: usize = 9;
...
fn writeConfigTable(bytes: []u8, cpu_count: u8) void {
    ...
    offset += interrupt_entry_bytes;
    writeInterrupt(bytes[offset..][0..interrupt_entry_bytes], 0, 11 << 2, io_apic_id, 14);
    offset += interrupt_entry_bytes;
...
test "MP tables publish each CPU and have valid checksums" {
    ...
    try std.testing.expectEqual(@as(u16, 31), std.mem.readInt(u16, table[34..36], .little));
    for (0..4) |index| {
