...
pub const P9Server = struct {
    ...
    pub fn handle(self: *P9Server, req: []const u8, resp: []u8) usize {
        ...
        return self.dispatch(msg_type, tag, &r, &w) catch |err| switch (err) {
            ...
            error.NoSpace => lerror(&w, tag, L_ENOSPC),
        };
    }
    ...
    fn dispatch(self: *P9Server, msg_type: u8, tag: u16, r: *Reader, w: *Writer) Error!usize {
        ...
        switch (msg_type) {
            ...
            Tgetattr => {
                ...
                try w.qid(qidFromStat(st));
                try w.u32v(statMode(st));
                try w.u32v(@intCast(st.st_uid));
                try w.u32v(@intCast(st.st_gid));
                try w.u64v(@intCast(st.st_nlink));
                try w.u64v(@bitCast(@as(i64, @intCast(st.st_rdev))));
                try w.u64v(@bitCast(@as(i64, @intCast(st.st_size))));
                try w.u64v(4096); // blksize
                try w.u64v(@bitCast(@as(i64, @intCast(st.st_blocks))));
                try w.u64v(@bitCast(statAtimeSec(st)));
                try w.u64v(@bitCast(statAtimeNsec(st)));
                try w.u64v(@bitCast(statMtimeSec(st)));
                try w.u64v(@bitCast(statMtimeNsec(st)));
                try w.u64v(@bitCast(statCtimeSec(st)));
                try w.u64v(@bitCast(statCtimeNsec(st)));
